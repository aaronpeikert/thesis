...
As psychology grapples with this crisis of confidence in its empirical findings, several causes have been identified and respective remedies have been proposed.
...
Empirical scientists, however, often simultaneously engage in induction by deriving general statements from past observation.
...
Therefore, transparency is more than a virtue that may improve empirical sciences somewhat but is an indispensable property.
...
Either the variance is known, than a purely deductive test is possible, or it has to be estimated from data, making induction part of the empirical test.
Consider a placebo and an experimental group; there is possible to hypothesize and test a mean difference using a simple t-test.
The decision to reject the null hypothesis (groups have equal means) depends on, besides true mean difference and sample size, the observed variance.
...
A t-test accounts for the estimation of variance by having somewhat wider tails than a z-test which assumes that the variance is known, i.e., threshold of significance is higher for a t-test (the critical value for a one-sided test with α-level set to 1% and degrees of freedom set to 10, is t≈`r round(qt(.99, 10), 2)` vs. z≈`r round(qnorm(.99), 2)`).
...
If they had to, there would be currently very few psychological theory that could be subjected to an empirical test [@muthukrishnaProblemTheory2019].
...
So empirical researchers find themselves in a predicament.
...
Both frameworks show how unaccounted induction leads to overconfidence in empirical results and imply some theoretical tools on how to disclose induction so that this overconfidence can be assessed and controlled.
...
The quantity of interst is how much information is lost if we use $g(x)$, our theory as description of the world..., instead of $f(x)$, the reality, over all possible states $\mathcal{X}$.
...
Many readers will recognize that this information-theoretic setup and the derivation below follow closely @burnhamModelSelectionMultimodel2002, Ch. 7.2, in their derivation of the Akaike Information Criterion [@hirotuguakaikeInformationTheoryExtension1971] in its general form.
...
This bias is often called the complexity or capacity of a model, i.e., how much the data are influencing the results, hence, how detailed the model is representing the data [@goodfellowCapacityOverfittingUnderfitting2016; @mikkelsonComplexityVerisimilitudeRealism2001].
...
So the observed loss is an underestimation that has to be corrected for by considering the complexity of the model.
\begin{equation}
...
The first requires a formal analysis of the statistical model and the inductive process it was embedded in, to compute the complexity (parametric, e.g., using information criteria), and the second merely requires that the process is repeatable (nonparametric, e.g., using cross-validation).
...
Even if it were not, the linear model could be fitted on a large set of other data to asses its expected inductive behavior nonparametrically.
However, suppose the researcher reconsiders the model based on the results and their internal cognitive model (including prior knowledge, expectations, cognitive biases).
...
In the following, I propose preregistration to move induction into the formal domain (pushing the first bound) and computational reproducibility to make formal induction transparent (pushing the second bound).
I argue that both practices should ultimately provide sufficient transparency to judge empirical support for theories, though they may have secondary benefits for science.
...
Given enough computing power, automatic reproducibility allows to scale the inductive process to many data sets and therefore enables the assesment of the complexity.
...
Fourth, how the data relates to software and computing infrastructure must be managed by, e.g., workflow automation [@feldmanMakeProgramMaintaining1979; @kimImplementingGitHubActions2022; @kinsmanHowSoftwareDevelopers2021].
...
Note that requiring induction to be provably formal (analytically analyzable or repeatable), does not imply that induction itself is limited in its extent.
...
In that case, many statistical models exist that allow almost arbitrary functional forms [@rissanenUniversalCodingInformation1984], e.g., smoothing splines [@cravenSmoothingNoisyData1978], random forests [@hoRandomSubspaceMethod1998], neural networks [@amariUniversalTheoremLearning1993].
If the theory does not suffice to define an outlier, this decision can be made data-dependent, etc.
...
When the data are gathered, the results are updated to reflect the actual observations.
...
Specifically, I propose that transparency must concern the extent to which data may influence the results and which is captured in the term complexity.
...
I find it unconvincing that with some more effort researchers will develop better and more precise theories from thin air.
...
It might happen that in the end, more than one inductive process may be applicable, and no criterion makes one preferable over the other, i.e., from the point of the researcher, the decision is arbitrary.
