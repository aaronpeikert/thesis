...
However, both setups show that the extent of the optimism is closely related to the complexity of the inductive process ($\mathcal{C}$) and give us two starting points for making this bias transparent.
...
However, suppose the researcher reconsiders the model based on the results and fits a model with, e.g., a predictor added or removed.
In that case, the complexity can not be formally judged because the researcher is part of the inductive process.
...
Therefore, we have two bounds that limit transparency about the inductive process.
First, some things can not be made transparent in principle because some induction happens informally, so we can not estimate the complexity with certainty.
Second, even formalized inductive processes only allow estimating optimism theoretically, but they have to be communicated effectively to do so in practice.
...
In that case, we still have transparency about the inductive process in the sense that we can estimate the complexity of the inductive process.
...
The most minimal extension of computational reproducibility is, therefore, to require additionally that the process can be repeated on other similar data.
...
Removing the human from the loop enables researchers to verify formally that the first requirement is met and drastically reduces the burden of the second requirement.
Given enough computing power, automation enables to run the same analysis on many data sets.

While this workflow provides transparency and facilitates that the recipients of the work can judge the inductive process quickly, it asks a lot from the researchers who create the work.
...
To address the problem of making transparent how the data led to the results, these works divide it into four sub problems.
...
If a researcher makes every effort to only employ formal inductive reasoning in a reproducible manner, they still must persuade their readers of this fact.
...
We could simply trust the researchers.
In fact, we would have to trust them on two accounts.
First, we must put faith in their honesty.
Second, we must trust their competence in deciding what to disclose how.
...
Relying on trust for this judgment is not entirely unreasonable.
However, while this type of trust is a requirement to judge the informal complexity, it does not distinguish a formal from an informal inductive process.
Simply trusting that researchers know about the pitfalls of informal induction and disclose the required information is a subjective judgment.
Relying on a subjective judgment would mean to surrender the advantage a formal inductive process provides; instead of quantification of complexity, the complexity must be judged with a considerable amount of uncertainty.

A clearer distinction between formal and informal can be provided by specifying the formal inductive process before any data is available.
...
Note that preregistration is about the flow of information, i.e., the data used for induction, and not about the temporal order.
...
While this makes registration so difficult, it is exactly the information we need.
The expectation of the inductive process over all possible data is what we are trying to estimate, when we adjusting for the complexity of the inductive process.
