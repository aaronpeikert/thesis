...
This observation is only of limited use if we do not know the extend of this optimism.
...
What we can do is to split $\mathcal{C}$ in complexity that can be formally described and a part that can only be evaluated subjectively.
...
However, if the researcher reconsiders the model based on the results, and fits a model with, e.g., a predictor added or removed, the complexity can not be formally judged because the induction is partly done by the researcher themselves.
...
This fact implies that for theses situations the complexity and hence the optimism bias can not be fully quantified.
The second best thing to transparency about the complexity is to communicate the uncertainty about the complexity clearly.
However, judging the informal complexity $\mathcal{C}(\text{informal})$ must remain an subjective exercise.
...
Informal inductive processes allow only a limited transparency and must be subjectively judged.
...
I argue that both practices are ultimately aimed at providing transparency to judge empirical support for theories, though they may have secondary benefits for science.

## Transparency about statistical models: Computational Reproducibilibiltiy

Assume for a moment that a researcher only engaged in formal induction while testing a theory empirically.
...
Consider a software that delivers the same results upon input of the same data, i.e., its reproducible.
...
The second condition prevents us from changing the data in any way and observe the results.
We, therefore, have no means to assess the complexity of the inductive process and can not judge the empirical support.
...
If we relax the second condition and have a "black box" that is nerveless repeatable on other data, we still have transparency about the inductive process, in the sense, that we can estimate the complexity of the inductive process.
Relaxing the first condition most likely leads to relaxing the second condition as well.
If the inductive process can be understood by humans, complexity can either be calculated analytically or the process can be reimplemented to work on other similar data.
...
Second, computational reproducibility must enable that the inductive process can be repeated on similar data.
...
Removing the human from the loop enables to formally verify that the first requirement is met, and drastically reduces the burden of the second requirement.
...
To address this problem, and make the application of the workflow easier I developed the R package `repro` [@R-repro].
...
Addressing these four problems through automation lowers the barrier to understand, and hence make transparent, the inductive process.
However, it also addresses the here proposed additional requirement for computational reproducibility, that the process can be repeated on similar data, as it effectively allows to scale the process across many data sets.
