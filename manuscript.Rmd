...
As a first step, it can be noted that one may split $\mathcal{C}$ in complexity that can be formally described and a part that can only be evaluated subjectively.
...
Even if it were not, the linear model could be fitted on a large set of other data to asses its inductive behavior.
...
The researcher cannot be asked to repeat the process on all possible data sets, nor is the behavior mathematically well-defined.
However, it is known that the resulting linear model is more complex than a linear regression formally implies.
