...
Because both frameworks are quite technical, they are followed by a more conceptual summary of these tools.
...
In the future performance setup, we do not appeal to ground truth, replace the Kullback–Leibler divergence with an arbitrary loss function, and no longer require $g(x)$ to return a likelihood ($\mathcal{L}$ stands for an arbitrary loss function):
\begin{equation}
...
However, there is another bound on transparency that we must consider.
...
In the following, I propose preregistration as means to move induction into the formal domain (pushing the first bound) and computational reproducibility (pushing the second bond) to make formal induction transparent.
...
In other words, having formal induction is not enough, the induction still must be made transparent.
...
Relaxing the first condition, intransparency to human researchers, most likely leads to relaxing the second condition, repeatability on other data, as well.
...
First, computational reproducibility must show that the results have been the product of the proposed inductive process, i.e., the same data leads to the same results.
Second, computational reproducibility must enable the inductive process to be repeated on similar data.
...
To address the problem automating reproducibility, these works divide it into four sub problems.
...
Addressing these four problems through automation lowers the barrier for assessing the inductive process.
However, it also addresses the proposed additional requirement for computational reproducibility, that the process can be repeated on similar data, as it effectively allows to scale the process across many data sets.
...
To judge the complexity, the process must be transparent to the researcher, i.e., how would the results change if the data looked different.
...
While this makes registration so difficult, it is exactly the information we need and from which we derive the complexity of the inductive process.
...
A formal inductive process can still be designed to accommodate a wide range of data patterns, e.g., for a novel research question or a vague theory.
...
Second, what happens when, despite all considerations, the data does not behave as anticipated or the registered method is deemed unsuitable because of other reasons?

In @peikertReproducibleResearchTutorial2021 I propose a practical, yet rigorous form of preregistration, termed preregistration as code (PAC).
...
When the data is then gathered the results are updated to reflect the actual data.

In XXX, I investigate the objective of preregistration and how it may be separated from the objective of confirmatory science.
Specifically, I propose reduction of uncertainty about theoretical risk as the formal objective of preregistration.
