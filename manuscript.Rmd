...
In other words, induction gives scientific theories some slack to be imprecise and contain "blank" spaces, later to be filled based on observation.
...
If pushed to the limit, the result is a "theory" governed by the data.
Such a theory, full of empirically induced statements, is almost empty of statements that have been empirically verified.
The data used for induction cannot refute these statements, so they have never been subject to an empirical test.
...
The replication crisis can be traced to a misjudgment of how much induction has been going on in psychology hence, how well-tested the empirical claims, as reported in the literature, actually are.
Therefore, the question of this dissertation is *what* has to be be made transparent, and *how* to best make it transparent?
...
When interested in how much information is lost when using $g(x)$, the theory, instead of $f(x)$, the reality, over all possible states $\mathcal{X}$.
Expressed as lost bits of information, a measure known as Kullback–Leibler divergence [@kullbackInformationSufficiency1951], this results:
...
What is of interest is not the derivation but how this conceptualization may facilitate to understand what happens when data is simultaneously used to induce quantities of a theory and to test the theory.

This setup is highly theoretical.
...
To allow quantities to be induced, we must assume that our theory is parameterized, e.g., $g(x|\theta)$.
...
It follows, almost surely, that we lose information:
\begin{equation}
...
We may conclude that substituting deduced quantities with induced estimates leads to some overconfidence about how close one is to the truth.
